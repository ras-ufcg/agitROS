...
{:toc}
---
Até agora, nós vimos um exemplo de programa que mostra mensagens. Esse é, com certeza, apenas metade da história quando se trata da comunicação com outros nós através de mensagens. Vamos agora dar uma olhada no programa que se [**inscrevem**](http://wiki.ros.org/ROS/Tutorials/WritingPublisherSubscriber(c++)) em mensagens publicadas por outros nós.
...
Uma diferença importante entre publishing e subscribing é que um nó de subscribing não sabe quando as mensagens chegarão. Para lidar com esse fato, devemos inserir qualquer código que responda a mensagens recebidas dentro de uma  **callback function**,
...
do tópico no qual nós pretendemos nos inscrever. O corpo da função de retorno então tem acesso a todos os campos na mensagem recebida, e pode armazenar, usar, ou descartar aquela informação como achar melhor. Como sempre, nós devemos incluir o cabeçalho apropriado que define essa classe.

No exemplo, nosso callback aceita mensagens do tipo  `turtlesim::Pose`, então o cabeçalho que precisamos é  `turtlesim/Pose.h`. 
(Nós podemos aprender que esse é o tipo de mensagem correto usando o `rostopic info`; relembrar a seção 2.7.2.) O callback apenas imprime alguns dados da mensagem, incluindo seus `x`, `y`, e `theta` membros de dados, por meio da `ROS_INFO_STREAM`. (Nós podemos aprender quais áreas de dados o tipo de mensagem tem usado
`rosmsg show`, novamente da seção 2.7.2.)
...
-	O ultimo parâmetro é um ponteiro para a função callback que o ROS deve executar quanto mensagens chegam.
Em C++, você pode levar um ponteiro a uma função usando o operador e-comercial (`&`, “endereço de”) no nome da função. No nosso exemplo, se parece com isto: 
```
 &poseMessageReceived
```
...
> quer levar um ponteiro a uma função sem chamá-la, como nós estamos fazendo aqui. O ROS supre os argumentos exigidos quando chama sua função callback
{: .fs-3 .code-example }

> ⏩ Comentário na sintaxe do C++: O e-comercial é na verdade opcional e muitos programas o omitem.
> O compilador pode dizer que você um ponteiro para a função, ao invés do valor devolvido pela execução
> da função, porque o nome da função não é seguido de parênteses. A sugestão do autor é inclui-lo, porque
> faz o fato que estamos lidando com um ponteiro mais óbvio para leitores humanos.
{: .fs-3 .code-example }

Você pode notar que, durante a criação do  `ros::Subscriber object`, nós não mencionamos explicitamente
o tipo da mensagem em parte alguma. De fato, o método subscribe é modelado e o compilador C++ deduz o
...
Um fato potencialmente contra intuitivo sobre `ros::Subscriber objects` é que deveras raro realmente chamar
qualquer um de seus métodos. Ao invés disso, a vida útil daquele objeto é a parte mais relevante:
Quando construímos um `ros::Subscriber`, our node establishes connections
...
ros::spinOnce();
```
Esse código pede que o ROS execute todos os callbacks pendentes de todas as inscrições dos nós e então **retornar o controle para nós**. A outra opção se parece com isto:
```
ros::spin();
```
...
> Omitir `ros::spin will` likely cause your program to exit shortly after it
> starts. Omitting `ros::spinOnce` pode fazer com que aperente que nenhuma mensagem está sendo recebida.
{: .fs-3 .code-example }
...
> ⚠️ Não esqueça de assegurar que seu pacote tem uma dependencia no `turtlesim`, …, o qual é necessário
> porque nós estamos usando o tipo de mensagem  `turtlesim/Pose`. Veja a seção 3.3.3 para lembrar como declarar essa dependência.
{: .fs-3 .code-example }
...
--- 
<a name="fn1">1</a> H Como nós sabemos que `turtlelism_node` atua nesse tópico? Uma forma de descobrir é iniciar esse nó e depois usar `rostopic list`, `rosnode info`, ou `rqt_graph`para ver os tópicos sendo editados. Veja a seção 2.7.1
