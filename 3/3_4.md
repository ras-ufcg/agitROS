...
topic, on which `turtlesim_node` publishes<sup>[1](#fn1)</sup>. Messages on this topic describe the **pose** — a
term referring to position and orientation — of the turtle. [Lista 3.5](#lista-35) shows a short program that subscribes to those messages and summarizes them for us via `ROS_INFO_STREAM`. Although some parts of this program should be familiar by now, there are three new elements.
...
> to the technique used for publishing messages but differs in
...
--- 
<a name="fn1">1</a> How do we know that turtlesim_node publishes on this topic? One way to find out is to start that node
