...
cabeçalho C++ correspondente. Você precisará `#include` esse cabeçalho para cada tipo de mensagem usada
...
odo s campos do tipo da mensagem fornecido. Esta classe é definida em um namespace nomeado depois
...
- A parte `message_type` dentro dos colchetes — formalmente chamado de modelo
...
A seção anterior cobriu os detalhes da publicação de mensagens. Nosso exemplo `pubvel` re-
pete os passos de publicação dentro de um loop `while` para publicar muitas mensagens diferentes conforme o 
...
o métodod "sleep" desse objeto.
```
rate.sleep();
```
Cada chamada a esse métodod causa um atraso no programa. A duração do atraso é calculada
...
Você pode confirmar que este regulamento está funcionando corretamente, usando rostopic hz. Para o `pub vel`, os resultados devem ser parecidos com isso:
```
average rate: 2.000
...
> ⚠️ Você deve estar pensando em uma alternativa par `ros::Rate` que use um simples, atraso fixo — 
...
### Declarando depedências do tipo de menssagem
