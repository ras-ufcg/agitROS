...
Nós iniciados com esse atributo exibirão a sua saída parão em tela ao invés dos arquivos 
de log discutivos acima. Esse exemplo usa o esse atributo para o nó `subpose`, o que explica
...
>⚠️ Se um programa, quando iniciado a partir do `roslaunch` aparenta não produzir 
>a saída que você espera, você deve verificar se aquele nó tem o atributo 
>If a program, when started from roslaunch, does not appear to be producing the `output="screen"` habilitado. 
{: .fs-3 .code-example }

### Solicitando respawning 

Após iniciar todos os nós requisitados, o `roslaunch` monitora cada nó, rastreando aqueles que permanecem ativos. 
Para cada nó, podemos solicitar o `roslaunch` que reinicie quanto finalizar, ao utilizar o atributo `respawn`: 
...
Quando um nó do tipo `required` termina, o `roslaunch` responde finalizando todos os outros nós ativos e finalizando ele mesmo. 
...
em um terminal em separado para cada nó, é que todos os nós compartilham o mesmo terminal. Isto é gerenciável (e muitas vezes útil)
...
para executar o nó em questão. No `example.launch`, utilizamos este atributo para o nó de teleoperação:
...
> [depuração](http://wiki.ros.org/rqt_console) (via `gdb` ou `valgrind`), ou para diminuir o nível de prioridade de um processo process (via `nice`).
{: .fs-3 .code-example }
